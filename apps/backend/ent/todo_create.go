...
func (tc *TodoCreate) SetTypes(s []string) *TodoCreate {
	tc.mutation.SetTypes(s)
	return tc
}
...
func (tc *TodoCreate) defaults() {
	if _, ok := tc.mutation.Types(); !ok {
		v := todo.DefaultTypes
		tc.mutation.SetTypes(v)
	}
...
func (tc *TodoCreate) createSpec() (*Todo, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.Types(); ok {
